...
def generate_text(seed, gpt2_model, gpt2_tokenizer, max_length=50, num_iterations=20, top_k=5, top_k_depth=3, context_window=5, punctuation_weight=0.1):
    ...
    for iteration in range(num_iterations):
        ...
        if current_length < max_length:
            # Add new [MASK] tokens at the edges to encourage expansion
            generated_tokens = [tokenizer.mask_token] + generated_tokens + [tokenizer.mask_token]
            current_length = len(generated_tokens)

        masked_positions = [i for i, token in enumerate(generated_tokens) if token == tokenizer.mask_token]
        print(f"Masked tokens: {generated_tokens}")
        ...
        for pos in masked_positions:
            left_context = generated_tokens[max(0, pos - context_window):pos]
            right_context = generated_tokens[pos + 1:min(len(generated_tokens), pos + context_window + 1)]
        ...
        for combination in torch.cartesian_prod(*infill_token_ids_list):
            infilled_tokens = generated_tokens.copy()
        ...
        if len(generated_tokens) >= max_length:
            print(f"Maximum length reached. Stopping generation.")
            break
...
if __name__ == '__main__':
    ...
    for seed in seed_words:
        print(f"\nSeed: {seed}")
        generated_text = generate_text(seed, gpt2_model, gpt2_tokenizer, max_length=2048, num_iterations=16, top_k=32, top_k_depth=16, context_window=16, punctuation_weight=0.25)
